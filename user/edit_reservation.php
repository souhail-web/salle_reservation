...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($room_id) || empty($reservation_date) || empty($start_time) || empty($end_time)) {
        ...
        try {
            // Check if the room is already booked for this time slot (excluding current reservation)
            $stmt = $pdo->prepare("SELECT id FROM reservations 
                                  ...
                                  OR (start_time >= ? AND end_time <= ?))
                                  AND id != ?");
            $stmt->execute([$room_id, $reservation_date, $end_time, $start_time, $start_time, $end_time, $start_time, $end_time, $reservation_id]);
            ...
            if ($stmt->rowCount() > 0) {
                ...
                $stmt->execute([$room_id, $reservation_date, $start_time, $end_time, $purpose, $reservation_id, $_SESSION['user_id']]);
...
<body>
    ...
    <div class="container mt-4">
        <h2><i class="fas fa-edit"></i> Modifier une réservation</h2>
        <p class="lead">Modifiez les détails de votre réservation ci-dessous.</p>
            ...
            <div class="card-body">
                <form method="POST">
                        ...
                        <div class="col-md-6">
                            <div class="mb-3">
                                ...
                                <select class="form-select" id="start_time" name="start_time" required>
                                    ...
                                    <option value="<?php echo $time; ?>" <?php echo ($time == substr($reservation['start_time'], 0, 5)) ? 'selected' : ''; ?>>
                                        <?php echo $time; ?>
                                    </option>
                                    <?php endforeach; ?>
                                </select>
                        ...
                        <div class="col-md-6">
                            <div class="mb-3">
                                ...
                                <select class="form-select" id="end_time" name="end_time" required>
                                    <option value="">Sélectionnez une heure</option>
                                    <?php foreach (getTimeOptions() as $time): ?>
                                    <option value="<?php echo $time; ?>" <?php echo ($time == substr($reservation['end_time'], 0, 5)) ? 'selected' : ''; ?>>
                                        <?php echo $time; ?>
                                    </option>
                                    <?php endforeach; ?>
                                </select>
                    ...
                    <div class="mb-3">
                        ...
                        <textarea class="form-control" id="purpose" name="purpose" rows="3"><?php echo htmlspecialchars($reservation['purpose']); ?></textarea>
                    </div>
                    
                    <div class="d-grid">
                        <button type="submit" class="btn btn-primary btn-lg">
                            <i class="fas fa-save"></i> Mettre à jour la réservation
                        </button>
                    </div>
                </form>
            </div>
